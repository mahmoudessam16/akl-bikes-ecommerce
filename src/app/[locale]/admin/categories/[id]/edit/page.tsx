...
export default function EditCategoryPage() {
  ...
  const fetchCategory = async () => {
    try {
      ...
      setImageSource(category.image?.includes('res.cloudinary.com') ? 'url' : category.image?.startsWith('data:') ? 'file' : 'url');
    } catch (error) {
  ...
  const mainCategories = categories.filter(c => !c.parentId && c.id !== categoryId);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20 lg:pb-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold">تعديل الفئة</h1>
          <p className="text-muted-foreground text-sm sm:text-base">قم بتعديل معلومات الفئة</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-w-4xl">
        ...
        <div className="flex gap-4 sticky bottom-0 bg-background border-t p-4 -mx-4 sm:mx-0 sm:border-0 sm:static sm:p-0">
          ...
          <Button type="submit" disabled={submitting || uploading} className="flex-1 sm:flex-initial cursor-pointer">
            {uploading ? 'جاري رفع الصورة...' : submitting ? 'جاري الحفظ...' : 'تحديث الفئة'}
          </Button>
        </div>
      </form>
    </div>
