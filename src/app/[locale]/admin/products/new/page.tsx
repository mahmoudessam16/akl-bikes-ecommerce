...
export default function NewProductPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const generatedId = `prod-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
      const generatedSku = `SKU-${Date.now()}-${Math.random().toString(36).substr(2, 6).toUpperCase()}`;
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20 lg:pb-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold">إضافة منتج جديد</h1>
          <p className="text-muted-foreground text-sm sm:text-base">أدخل معلومات المنتج الجديد</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-w-4xl">
        ...
        {/* Images Section - Similar structure to original but responsive */}
        <div className="bg-card rounded-lg border p-4 sm:p-6 space-y-4">
